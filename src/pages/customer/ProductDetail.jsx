    ...
    if (loading) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-pulse">
                <div className="aspect-square rounded-2xl bg-hover border border-border" />
                <div className="flex flex-col gap-4">
                    <div className="h-4 w-24 rounded bg-hover" />
                    <div className="h-9 w-3/4 rounded bg-hover" />
                    <div className="h-8 w-32 rounded bg-hover" />
                    <div className="h-20 rounded bg-hover" />
                    <div className="h-11 rounded bg-hover" />
                </div>
            </div>
        );
    }
    if (!product) return <p className="text-text-muted font-serif text-sm">Product not found.</p>;
    ...
    return (
        <div className="flex flex-col gap-8">
            ...
            <div className="flex items-center justify-between gap-3">
                ...
                <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm font-serif text-text-muted hover:text-text transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <div className="md:sticky md:top-24 flex flex-col gap-3">
                    <GlassCard>
                        <div className="aspect-square w-full bg-hover rounded-2xl overflow-hidden relative">
                            {product.image ? (
                                <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-text-muted font-serif text-sm">
                                    No image available
                                </div>
                            )}
                        ...
                        <div className="grid grid-cols-3 divide-x divide-border">
                            {[
                                ...
                                <div key={I.label} className="flex flex-col items-center gap-1.5 text-center py-4 px-2">
                                    ...
                                    <span className="text-[11px] font-serif text-text-muted leading-tight">{I.label}</span>
                                </div>
                            ))}
                        </div>
                    </GlassCard>
                ...
                <div className="flex flex-col gap-5">
                    <div className="flex flex-col gap-2.5">
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {product.brand && (
                                <span className="font-mono text-[11px] uppercase tracking-wider text-text-muted">{product.brand}</span>
                            )}
                        </div>
                        <h1 className="font-display italic text-3xl sm:text-4xl font-semibold text-text leading-tight">{product.name}</h1>
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-serif text-text-muted">Customer favourite</span>
                        </div>

                        <p className="text-sm font-serif text-text-muted leading-relaxed">{product.description}</p>
                    </div>
                        ...
                        <div className="p-4 flex flex-col gap-3">
                            <div className="flex items-end justify-between flex-wrap gap-2">
                                <div className="flex items-baseline gap-3">
                                    <span className="font-mono text-3xl text-text">${discounted.toFixed(2)}</span>
                                    {product.discount > 0 && (
                                        <span className="text-base font-mono text-text-muted line-through">${product.price.toFixed(2)}</span>
                                    )}
                                </div>
                                {product.discount > 0 && (
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-1">
                                <div className="flex items-center justify-between sm:justify-start gap-4 px-3 py-2.5 rounded-lg border border-border bg-hover">
                                    <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="p-1 text-text-muted hover:text-orange-400 transition-colors">
                                        <Minus size={14} />
                                    </button>
                                    <span className="font-mono text-sm text-text w-6 text-center">{qty}</span>
                                    <button onClick={() => setQty((q) => Math.min(product.stock, q + 1))} className="p-1 text-text-muted hover:text-orange-400 transition-colors">
                                        <Plus size={14} />
                                    </button>
                                </div>
                    ...
                    <GlassCard>
                        <div className="p-4 sm:p-5">
                            <p className="text-xs font-serif uppercase tracking-widest text-text-muted mb-3.5 pb-3 border-b border-border">
                                Specifications
                                {specEntries.length > 0 && (
                                    <span className="ml-1.5 text-[10px] font-mono text-text-muted">({specEntries.length})</span>
                                )}
                            </p>
                            <div className="flex flex-col">
                                {specEntries.length === 0 ? (
                                    <p className="text-sm font-serif text-text-muted">No specifications listed.</p>
                                ) : specEntries.map(([k, v]) => (
                                    <div key={k} className="flex items-center justify-between text-sm font-serif border-b border-border py-2.5 last:border-0">
