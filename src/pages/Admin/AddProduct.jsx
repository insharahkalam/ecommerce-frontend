  ...
  return (
    <>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display italic text-2xl font-semibold tracking-tight text-text">Products</h1>
          <p className="text-sm mt-1 text-text-muted font-serif">{products.length} products in catalog</p>
        </div>
              ...
              <div className="p-5 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => openEdit(p)} className="p-1.5 rounded-md hover:bg-hover text-text-muted hover:text-text transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => removeProduct(p.id)} className="p-1.5 rounded-md hover:bg-hover text-text-muted hover:text-red-400 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                <div>
                  <p className="text-sm font-serif text-text">{p.name}</p>
                  <p className="font-mono text-xs text-text-muted mt-0.5">{p.id} · {p.category}{p.brand ? ` · ${p.brand}` : ""}</p>
                </div>
                <div className="flex items-center justify-between">
                  <span className="font-mono text-lg text-text">Rs {p.price}</span>
                  <Pill color={st.color} bg={st.bg} border={st.border}>{st.label === "In stock" ? `${p.stock} in stock` : st.label}</Pill>
                </div>
                <p className="font-mono text-xs text-text-muted">{p.sold} sold all-time{p.discount ? ` · ${p.discount}% off` : ""}</p>
              </div>
            </GlassCard>
          );
        })}
        {filtered.length === 0 && (
          <div className="col-span-full text-center py-10 text-text-muted font-serif text-sm">No products match your search.</div>
        )}
      </div>
