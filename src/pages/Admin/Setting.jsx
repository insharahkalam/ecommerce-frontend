...
// One toggle row + its OWN independent save button.
// `dirty` = current value differs from what's actually saved on the server.
function ToggleRow({ label, desc, value, onChange, disabled, dirty, saving, saved, onSave }) {
  return (
    <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 px-4 py-3">
      ...
      {/* Per-toggle save button — only shows once this specific toggle differs from saved value */}
      {dirty && (
...
const FIELDS = ["notifyOrders", "notifyStock", "notifySignups"];

export default function Setting() {
  const [fullName, setFullName] = useState("");
  const [email, setEmail] = useState("");

  // Local (editable) values shown in the UI
  const [notifyOrders, setNotifyOrders] = useState(true);
  const [notifyStock, setNotifyStock] = useState(true);
  const [notifySignups, setNotifySignups] = useState(true);

  // What's actually confirmed-saved on the server — used to detect "dirty" state per field
  ...
  // Per-field saving / saved-flash state
  const [savingField, setSavingField] = useState(null); // which field is currently saving
  const [savedField, setSavedField] = useState(null); // which field just finished saving (for the checkmark flash)
  ...
  const setters = {
    ...
    notifyStock: setNotifyStock,
    notifySignups: setNotifySignups,
  };

  // Saves ONLY the one field that was changed — the backend PATCH already
  // supports partial updates (only overwrites keys that are !== undefined)
