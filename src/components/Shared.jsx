...
export function Logo2() {
    return (
        ...
        <Link to="/" className="group flex shrink-0 items-center gap-2">
            ...
            <span className="flex flex-col leading-none">
                <span className="font-serif text-[19px] font-extrabold tracking-[-0.03em] dark:text-white">
                    Urban<span className="bg-gradient-to-r from-orange-400 to-amber-300 bg-clip-text text-transparent">Traders</span>
                </span>
                <span className="mt-1 hidden text-[9px] font-semibold uppercase tracking-[0.28em] text-neutral-400 dark:text-neutral-500 sm:block">
                    Shop Smart
...
export function SocialButton({ icon, label }) {
    return (
        <button
            type="button"
            className="flex-1 flex items-center justify-center gap-2 border border-neutral-700 rounded-lg py-2.5 text-sm text-neutral-200 hover:bg-neutral-800 transition-colors"
        >
            {icon}
            {label}
        </button>
